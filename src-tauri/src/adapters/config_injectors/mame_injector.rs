...
impl EmulatorConfigInjector for MameInjector {
    ...
    async fn read_current(&self, config_dir: &str) -> Result<EmulatorSettings, String> {
        ...
        let content =
            fs::read_to_string(&ini_path).map_err(|e| format!("Cannot read mame.ini: {}", e))?;
        ...
        for line in &lines {
            ...
            match key {
                ...
                "keepaspect" => {
                    settings.video.aspect_ratio = Some(if val == "1" {
                        "4:3".into()
                    } else {
                        "stretch".into()
                    })
                ...
                "sound" => {
                    settings.audio.backend = Some(if val == "1" {
                        "enabled".into()
                    } else {
                        "disabled".into()
                    })
                ...
                _ => {
                    settings.advanced.insert(key.to_string(), val.to_string());
                }
            }
        }
    ...
    async fn inject(&self, config_dir: &str, settings: &EmulatorSettings) -> Result<(), String> {
        ...
        fs::write(&ini_path, content).map_err(|e| format!("Cannot write mame.ini: {}", e))?;
