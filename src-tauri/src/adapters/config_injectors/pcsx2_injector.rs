...
impl EmulatorConfigInjector for Pcsx2Injector {
    ...
    async fn read_current(&self, config_dir: &str) -> Result<EmulatorSettings, String> {
        ...
        let content =
            fs::read_to_string(&ini_path).map_err(|e| format!("Cannot read PCSX2.ini: {}", e))?;
        ...
        for line in content.lines() {
            ...
            if let Some((key, val)) = trimmed.split_once('=') {
                ...
                match k {
                    ...
                    "SynchronizeMode" => {
                        settings.audio.latency = Some(if v == "0" { 30 } else { 100 })
                    }
                    _ => {
    ...
    async fn inject(&self, config_dir: &str, settings: &EmulatorSettings) -> Result<(), String> {
        ...
        fs::write(&ini_path, &content).map_err(|e| format!("Cannot write PCSX2.ini: {}", e))?;
