...
impl EmulatorConfigInjector for RetroArchInjector {
    ...
    fn display_name(&self) -> &str { "RetroArch" }
    ...
    fn get_config_dir(&self, _emulator_path: &str) -> String {
        #[cfg(windows)]
        {
            ...
            PathBuf::from(local_app_data).join("RetroArch").to_string_lossy().to_string()
        }
        #[cfg(not(windows))]
        {
            let home = std::env::var("HOME").unwrap_or_else(|_| ".".to_string());
            PathBuf::from(home).join(".config/retroarch").to_string_lossy().to_string()
        }
    }
    ...
    async fn inject(&self, config_dir: &str, settings: &EmulatorSettings) -> Result<(), String> {
        ...
        if let Some(fs) = settings.video.fullscreen {
            content = set_cfg_value(&content, "video_fullscreen", if fs { "true" } else { "false" });
        }
        if let Some(vsync) = settings.video.vsync {
            content = set_cfg_value(&content, "video_vsync", if vsync { "true" } else { "false" });
        }
        ...
        fs::write(&cfg_path, &content)
            .map_err(|e| format!("Cannot write retroarch.cfg: {}", e))?;
...
fn set_cfg_value(content: &str, key: &str, value: &str) -> String {
    ...
    let result: Vec<String> = content.lines().map(|line| {
        let trimmed = line.trim();
        if trimmed.starts_with('#') { return line.to_string(); }
