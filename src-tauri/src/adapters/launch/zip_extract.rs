    ...
    async fn launch(&self, ctx: &LaunchContext) -> Result<LaunchResult> {
        ...
        #[cfg(windows)]
        {
            ...
            let ps_script = format!(
                ...
                rom_str, temp_dir.to_string_lossy()
            );
        ...
        match first_rom {
            Some(rom) => {
                let child = ctx.emulator_path.as_ref().map(|emu| {
...
fn find_first_rom(dir: &std::path::Path) -> Option<std::path::PathBuf> {
    let rom_exts = ["nes", "snes", "smc", "fig", "gen", "bin", "iso", "cue", "gba", "gb", "n64", "z64"];
